...
| :--- | :--- | :--- | :--- |
| **[Módulo 1](exercises/module1/README.md)** | **Ataques Offline** | 45 min | Fuerza bruta con **Hashcat** y Name-That-Hash. |
...
La mayoría de herramientas (hydra, hashcat, scripts python) están instaladas **dentro** del contenedor `attacker`.
...
> ⚠️ **Importante**: Si intentas ejecutar `python3` o `hashcat` directamente en tu terminal (fuera de Docker), podría funcionar si los tienes instalados, pero **no tendrán acceso a la red interna del taller** (no verán a `vulnerable-api` ni `ssh-target`). Usa siempre `docker-compose exec attacker ...`.
